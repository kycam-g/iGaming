...
function applyAdminBranding(settings={}){const site=String(settings.site_name||'MZ90').trim()||'MZ90';const brandName=$('#admin-brand-name');if(brandName){brandName.replaceChildren();const first=document.createElement('span');first.className='admin-brand-first';first.textContent=site.charAt(0);const rest=document.createElement('span');rest.className='admin-brand-rest';rest.textContent=site.slice(1);brandName.append(first,rest);}document.title=`${site} Admin`;if(settings.favicon_path!==undefined)setAdminBrowserFavicon(settings.favicon_path)}
...
function renderGateway(g){const node=$('#gateway-template').content.firstElementChild.cloneNode(true);node.dataset.code=g.code;node.dataset.enabled=g.enabled?'1':'0';$('h2',node).textContent=g.name;$('.gateway-code',node).textContent=g.code;$('[name=name]',node).value=g.name;$('[name=enabled]',node).checked=!!Number(g.enabled);$('[name=deposit_enabled]',node).checked=!!Number(g.deposit_enabled);$('[name=withdrawal_enabled]',node).checked=!!Number(g.withdrawal_enabled);$('[name=mode]',node).value=g.mode;$('[name=priority_deposit]',node).value=g.priority_deposit;$('[name=priority_withdrawal]',node).value=g.priority_withdrawal;$('[name=min_deposit]',node).value=minorToMoney(g.min_deposit_minor);$('[name=max_deposit]',node).value=minorToMoney(g.max_deposit_minor);$('[name=min_withdrawal]',node).value=minorToMoney(g.min_withdrawal_minor);$('[name=max_withdrawal]',node).value=minorToMoney(g.max_withdrawal_minor);$('.credential-state',node).textContent=g.credentials_configured?'Credenciais configuradas':'Sem credenciais';if(g.code==='pixup'){const b=$('.pixup-fields',node);b.classList.remove('hidden');$('[name=base_url]',node).value=g.settings?.base_url||'https://api.pixupbr.com';$('[name=webhook_url]',node).value=g.settings?.webhook_url||'';$('[name=verify_webhook_signature]',node).checked=!!g.settings?.verify_webhook_signature}if(g.code==='abilitypay'){const b=$('.abilitypay-fields',node);b.classList.remove('hidden');$('[name=abilitypay_base_url]',node).value=g.settings?.base_url||'https://abilitypay.app/api';$('[name=abilitypay_deposit_description]',node).value=g.settings?.deposit_description||'Depósito PIX';$('[name=abilitypay_withdrawal_description]',node).value=g.settings?.withdrawal_description||'Saque da plataforma';const origin=location.origin;$('[name=abilitypay_callback_url]',node).value=origin+'/api/abilitypay/callback'}if(g.code==='bspay'){const b=$('.bspay-fields',node);b.classList.remove('hidden');$('[name=bspay_base_url]',node).value=g.settings?.base_url||'https://api.bspay.co';$('[name=bspay_withdrawal_description]',node).value=g.settings?.withdrawal_description||'Saque da plataforma';$('[name=bspay_webhook_url]',node).value=g.settings?.webhook_url||'';const origin=location.origin;$('[name=bspay_callback_url]',node).value=origin+'/api/bspay/callback'}$('.save-gateway',node).addEventListener('click',()=>saveGateway(node,g.code));return node}
async function saveGateway(node,code){const q=n=>$(`[name=${n}]`,node);const payload={code,name:q('name').value,enabled:q('enabled').checked,deposit_enabled:q('deposit_enabled').checked,withdrawal_enabled:q('withdrawal_enabled').checked,mode:q('mode').value,priority_deposit:Number(q('priority_deposit').value||100),priority_withdrawal:Number(q('priority_withdrawal').value||100),min_deposit_minor:moneyToMinor(q('min_deposit').value)||0,max_deposit_minor:moneyToMinor(q('max_deposit').value),min_withdrawal_minor:moneyToMinor(q('min_withdrawal').value)||0,max_withdrawal_minor:moneyToMinor(q('max_withdrawal').value),credentials:{},settings:{}};if(code==='pixup'){payload.credentials={client_id:q('client_id').value,client_secret:q('client_secret').value,webhook_secret:q('webhook_secret').value};payload.settings={base_url:q('base_url').value,webhook_url:q('webhook_url').value,verify_webhook_signature:q('verify_webhook_signature').checked}}if(code==='abilitypay'){payload.credentials={client_id:q('abilitypay_client_id').value,client_secret:q('abilitypay_client_secret').value};payload.settings={base_url:q('abilitypay_base_url').value,deposit_description:q('abilitypay_deposit_description').value,withdrawal_description:q('abilitypay_withdrawal_description').value}}if(code==='bspay'){payload.credentials={client_id:q('bspay_client_id').value,client_secret:q('bspay_client_secret').value,signing_key:q('bspay_signing_key').value};payload.clear_credentials=['webhook_secret'];payload.settings={base_url:q('bspay_base_url').value,webhook_url:q('bspay_webhook_url').value.trim(),withdrawal_description:q('bspay_withdrawal_description').value}}try{await api('/admin/api/gateways/save',{method:'POST',body:JSON.stringify(payload)});alertBox('#save-alert',`Gateway ${code} salvo.`,true);await loadGateways()}catch(e){alertBox('#save-alert',e.message)}}
boot();

let casinoCatalog={providers:[],categories:[],games:[]};
let casinoGamePage=1;
const casinoApiLabel=source=>source==='PLAYFIVER'?'PlayFiver':source==='GAMES2API'?'Games2API':'Catálogo local';
...
function casinoGameOptions(){
 ...
 const sources=[...new Set(casinoCatalog.games.map(v=>v.api_source||'MANUAL'))];
...
function renderCasinoGames(){
 ...
 for(const game of items.slice(start,start+pageSize)){
   ...
   const apiCell=cell(casinoApiLabel(game.api_source),'game-api-source');apiCell.classList.add(game.api_source==='PLAYFIVER'?'is-playfiver':game.api_source==='GAMES2API'?'is-games2api':'is-manual');
...
(() => {
 ...
 const choose=(tab,expand=true)=>{
   ...
   const description=tab==='providers'?'Gerencie os provedores cadastrados.':tab==='categories'?'Organize Slots, Pescaria, SportBet, Roleta e outras categorias.':tab==='credentials'?'Credenciais das APIs de jogos, ativação exclusiva, game launch e callbacks financeiros.':'Busca, filtros e visibilidade do catálogo.';
...
$('#playfiver-form').addEventListener('submit',async e=>{e.preventDefault();const f=e.currentTarget;const btn=f.querySelector('[type="submit"]');btn.disabled=true;try{const data=Object.fromEntries(new FormData(f));data.enabled=f.elements.enabled.checked;await api('/admin/api/casino/playfiver',{method:'POST',body:JSON.stringify(data)});for(const name of ['agent_code','agent_token','agent_secret'])f.elements[name].value='';await Promise.all([loadPlayfiverConfig(),loadGames2ApiConfig()]);await loadCasino();alertBox('#playfiver-alert','Integração PlayFiver salva. A ativação exclusiva foi aplicada.',true)}catch(err){alertBox('#playfiver-alert',err.message)}finally{btn.disabled=false}});
document.querySelector('[data-casino-tab="credentials"]').addEventListener('click',loadPlayfiverConfig);
async function loadGames2ApiConfig(){try{const d=await api('/admin/api/casino/games2api');const c=d.config,f=$('#games2api-form');f.elements.base_url.value=c.base_url||'https://api.games2api.xyz';f.elements.enabled.checked=!!c.enabled;$('#games2api-status').textContent=c.configured?(c.enabled?'Games2API ativa':'Games2API configurada, porém desativada'):'Credenciais Games2API não configuradas';const cb=$('#games2api-callback-url');if(cb)cb.textContent=location.origin+(window.IGAMING?.basePath||'')+'/api/webhooks/casino/games2api';}catch(e){alertBox('#games2api-alert',e.message)}}
$('#games2api-form').addEventListener('submit',async e=>{e.preventDefault();const f=e.currentTarget,btn=f.querySelector('[type=submit]');btn.disabled=true;try{const data=Object.fromEntries(new FormData(f));data.enabled=f.elements.enabled.checked;await api('/admin/api/casino/games2api',{method:'POST',body:JSON.stringify(data)});for(const name of ['agent_code','agent_token','agent_secret'])f.elements[name].value='';await Promise.all([loadGames2ApiConfig(),loadPlayfiverConfig()]);await loadCasino();alertBox('#games2api-alert','Integração Games2API salva. A ativação exclusiva foi aplicada.',true)}catch(err){alertBox('#games2api-alert',err.message)}finally{btn.disabled=false}});
$('#games2api-sync').addEventListener('click',async()=>{const btn=$('#games2api-sync');btn.disabled=true;try{const d=await api('/admin/api/casino/games2api/sync',{method:'POST',body:'{}'});await loadCasino();alertBox('#games2api-alert',`Catálogo sincronizado: ${d.providers_synced||0} provedores e ${d.games_synced||0} jogos.`,true)}catch(err){alertBox('#games2api-alert',err.message)}finally{btn.disabled=false}});
document.querySelector('[data-casino-tab="credentials"]').addEventListener('click',loadGames2ApiConfig);

// Plataforma: controles administrativos autenticados, sem mutação de saldo.
let platformData={};let auditPage=1;
async function loadPlatform(){try{platformData=await api('/admin/api/platform');applyAdminBranding(platformData.settings||{});const f=$('#settings-form');for(const [k,v] of Object.entries(platformData.settings)){if(!f.elements[k])continue;if(f.elements[k].type==='checkbox')f.elements[k].checked=v==='1';else f.elements[k].value=v;}for(const type of ['banners','affiliates'])renderPlatform(type);renderAnnouncements();renderFloatingIcons();populateIdentity()}catch(e){alertBox('#settings-alert',e.message)}}

function floatingModuleLabel(id){return ({rewards_center:'Central de Recompensas',agency:'Agência e Indicações',chests:'Baú do Tesouro',checkin:'Check-in',vip:'Clube VIP',rescue:'Fundos de Resgate',roulette:'Giro da Sorte',cashwheel:'Roleta de Saque',lottery:'Sorteio de Cartas',coupons:'Troca de Recompensas',rebate:'Rebate'})[id]||'Nenhum';}
function floatingShowLabel(value){return ({always:'Sempre',any_reward:'Qualquer recompensa',module_available:'Módulo disponível'})[value]||value;}
function renderFloatingIcons(){
 const host=$('#floating-icons-list');if(!host)return;host.replaceChildren();const rows=[...(platformData.floating_icons||[])].sort((a,b)=>Number(a.sort_order)-Number(b.sort_order));
 if(!rows.length){host.innerHTML='<div class="appearance-empty">Nenhum ícone flutuante cadastrado. Use “Novo ícone” para começar.</div>';return;}
 const table=document.createElement('table');table.className='appearance-table';table.innerHTML='<thead><tr><th>Prévia</th><th>Título</th><th>Módulo</th><th>Exibição</th><th>Link</th><th>Status</th><th>Ordem</th><th>Ações</th></tr></thead>';const body=document.createElement('tbody');
 for(const item of rows){const tr=document.createElement('tr');const preview=document.createElement('td');if(item.image_path){const img=document.createElement('img');img.src=item.image_path;img.alt=item.title;img.className='floating-admin-preview';preview.append(img)}else preview.textContent='🎁';tr.append(preview);const cell=v=>{const td=document.createElement('td');td.textContent=v;tr.append(td)};cell(item.title);cell(floatingModuleLabel(item.module_id));cell(floatingShowLabel(item.show_when));cell(item.link_path||'—');const status=document.createElement('td');const badge=document.createElement('span');badge.className='appearance-status '+(Number(item.enabled)?'on':'off');badge.textContent=Number(item.enabled)?'Ativo':'Inativo';status.append(badge);tr.append(status);cell(String(item.sort_order));const actions=document.createElement('td');const edit=document.createElement('button');edit.type='button';edit.className='secondary';edit.textContent='Editar';edit.onclick=()=>editFloatingIcon(item);const del=document.createElement('button');del.type='button';del.className='danger-button';del.textContent='Excluir';del.onclick=()=>deleteFloatingIcon(item);actions.append(edit,del);tr.append(actions);body.append(tr)}
 table.append(body);host.append(table);
}
function editFloatingIcon(item){const f=$('#floating-icons-form');f.reset();for(const [k,v] of Object.entries(item)){const el=f.elements[k];if(!el||el.type==='file')continue;if(el.type==='checkbox')el.checked=!!Number(v);else el.value=v??'';}$('#editor-title-floating-icons-form').textContent='Editar ícone flutuante';openEditor('floating-icons-form',true);}
async function deleteFloatingIcon(item){if(!confirm(`Excluir o ícone “${item.title}”?`))return;try{await api('/admin/api/platform/floating-icons/delete',{method:'POST',body:JSON.stringify({id:item.id})});await loadPlatform();alertBox('#floating-icons-alert','Ícone excluído.',true)}catch(e){alertBox('#floating-icons-alert',e.message)}}
$('#new-floating-icon')?.addEventListener('click',()=>{const f=$('#floating-icons-form');f.reset();f.elements.id.value='';f.elements.image_path.value='';f.elements.sort_order.value='100';f.elements.enabled.checked=true;$('#editor-title-floating-icons-form').textContent='Novo ícone flutuante';openEditor('floating-icons-form',false)});
$('#floating-icons-form')?.addEventListener('submit',async e=>{e.preventDefault();const f=e.currentTarget;const d=Object.fromEntries(new FormData(f));d.enabled=f.elements.enabled.checked;const submit=f.querySelector('[type=submit]');submit.disabled=true;try{const file=f.elements.image.files[0];if(file){const fd=new FormData();fd.append('kind','floats');fd.append('image',file);const r=await fetch('/admin/api/platform/upload',{method:'POST',headers:{Authorization:`Bearer ${token}`},body:fd});const out=await r.json();if(!r.ok)throw new Error(out.message||out.error||'Upload falhou');d.image_path=out.path;}await api('/admin/api/platform/floating-icons/save',{method:'POST',body:JSON.stringify(d)});closeEditor('floating-icons-form');f.reset();await loadPlatform();alertBox('#floating-icons-alert','Ícone flutuante salvo.',true)}catch(error){const box=$('#editor-floating-icons-form .admin-editor-error');if(box){box.textContent=error.message;box.classList.remove('hidden')}alertBox('#floating-icons-alert',error.message)}finally{submit.disabled=false}});
...
(() => {
 ...
 const centralPanels=[...document.querySelectorAll('.mz-promo-central-panel')];
 const titles={central:'Central de Promoções',vip:'Níveis VIP',coupons:'Cupons',checkin:'Check-in diário',roulette:'Giro da Sorte',envelope:'Envelope vermelho',chests:'Baús e indicações',agency:'Agência',rebate:'Rebate',rescue:'Fundos de Resgate',cashwheel:'Roleta de Saque',lottery:'Sorteio','bonus-history':'Histórico de bônus','level-history':'Histórico de níveis'};
 const select=(key)=>{
  const isCentral=key==='central';
  document.querySelectorAll('[data-admin-promotion-tab]').forEach(b=>b.classList.toggle('active',b.dataset.adminPromotionTab===key));
  centralPanels.forEach(panel=>panel.classList.toggle('hidden',!isCentral));
  detail.classList.toggle('hidden',isCentral||!key);campaigns?.classList.toggle('hidden',!!key);
  if(isCentral){setHeading('PROMOÇÕES','Central de Promoções','Visão geral, controles, diagnósticos e auditoria das campanhas.');return;}
 ...
 parent?.addEventListener('click',e=>{e.stopImmediatePropagation();const collapsed=group.classList.toggle('collapsed');parent.setAttribute('aria-expanded',String(!collapsed));if(!collapsed){openPage('promotions');select('central')}});
...
(() => {
 const definitions={
  ...
  lottery:[['spins_per_day','Rodadas por dia','integer'],['hit_chance_percent','Dificuldade / chance de carta útil','chance_level'],['collection_bonus_cents','Prêmio por coleção completa (R$)','money'],['rollover_x','Rollover (x)','decimal']]
 };
 ...
 const val=(value,kind)=>kind==='money'?brl(value):kind==='bool'?(Number(value)?'Sim':'Não'):kind==='chance_level'?({80:'Muito fácil (80%)',65:'Fácil (65%)',50:'Normal (50%)',35:'Difícil (35%)',20:'Muito difícil (20%)'}[Number(value)]||String(value??'—')+'%'):String(value??'—');
 ...
 const open=item=>{
  ...
  for(const [key,label,kind] of definitions[active]){
   ...
   else if(kind==='chance_level'){control.replaceChildren();for(const [v,t] of [['80','Muito fácil — 80% carta útil'],['65','Fácil — 65% carta útil'],['50','Normal — 50% carta útil'],['35','Difícil — 35% carta útil'],['20','Muito difícil — 20% carta útil']]){const option=document.createElement('option');option.value=v;option.textContent=t;control.append(option)}control.value=String(item?.config?.[key]??50)}
 ...
 const load=async()=>{
  ...
  try{const data=await api('/admin/api/promotion-configs?type='+encodeURIComponent(active));if(!definitions[active])return;items=data.items||[];render();
    ...
    if(active==='roulette'||active==='cashwheel'||active==='lottery'){
      addButton.disabled=items.length>0;
      const singleName=active==='roulette'?'Giro da Sorte':active==='cashwheel'?'Roleta de Saque':'Sorteio';
 ...
 const render=()=>{
  ...
  const visibleDefinitions=active==='lottery'?definitions[active].filter(([key])=>key!=='hit_chance_percent'):definitions[active];
  const tr=document.createElement('tr');for(const label of ['Nome',...visibleDefinitions.map(x=>x[1]),'Status','Ações']){const th=document.createElement('th');th.textContent=label;tr.append(th)}head.append(tr);
  if(!items.length){const row=document.createElement('tr'),cell=document.createElement('td');cell.colSpan=visibleDefinitions.length+3;cell.textContent='Nenhuma configuração cadastrada.';cell.className='promotion-empty';row.append(cell);body.append(row);return;}
  for(const item of items){const row=document.createElement('tr');for(const text of [item.title,...visibleDefinitions.map(([k,,type])=>val(item.config?.[k],type)),Number(item.enabled)?'Habilitada':'Desabilitada']){const cell=document.createElement('td');cell.textContent=text;row.append(cell)}
 ...
 async function loadLotteryReport(){
   let panel=document.getElementById('mz-lottery-admin-report');
   if(!panel){panel=document.createElement('section');panel.id='mz-lottery-admin-report';panel.className='panel';document.querySelector('#admin-promotion-detail').append(panel);}
   panel.replaceChildren();const heading=document.createElement('h3');heading.textContent='Participações • Sorteio de Cartas';panel.append(heading);
   const note=document.createElement('p');note.className='hint';note.textContent='As letras podem se repetir e acumular. A coleção exige no mínimo H x1, A x1, P x2 e Y x1; a última letra necessária é extremamente rara.';panel.append(note);
   try{const data=await api('/admin/api/promotions/lottery/report');
     if(!data.players?.length){panel.append(document.createTextNode('Nenhuma participação registrada ainda.'));return;}
     const wrap=document.createElement('div');wrap.className='table-wrap';const table=document.createElement('table');table.innerHTML='<thead><tr><th>Jogador</th><th>Coleção atual</th><th>Coleções concluídas</th><th>Atualizado</th></tr></thead><tbody></tbody>';const body=table.querySelector('tbody');
     for(const row of data.players){let c={};try{c=JSON.parse(row.collection_json||'{}')}catch{}const progress=`H x${c.H||0} · A x${c.A||0} · P x${c.P||0} · Y x${c.Y||0}`;const tr=document.createElement('tr');tr.innerHTML=`<td>${escapeHtml(row.username)}</td><td>${escapeHtml(progress)}</td><td>${Number(row.completed_collections||0)}</td><td>${dateTime(row.updated_at)}</td>`;body.append(tr);}wrap.append(table);panel.append(wrap);
   }catch(error){panel.append(document.createTextNode('Falha ao carregar o Sorteio: '+error.message));}
 }
 window.promotionConfigSelect=key=>{active=key;const vipPanel=document.getElementById('mz-vip-admin-settings');if(vipPanel)vipPanel.classList.toggle('hidden',key!=='vip');const chestPanel=document.getElementById('mz-chest-admin-report');if(chestPanel)chestPanel.classList.toggle('hidden',key!=='chests');const agencyPanel=document.getElementById('mz-agency-admin-report');if(agencyPanel)agencyPanel.classList.toggle('hidden',key!=='agency');const rebatePanel=document.getElementById('mz-rebate-admin-report');if(rebatePanel)rebatePanel.classList.toggle('hidden',key!=='rebate');const rescuePanel=document.getElementById('mz-rescue-admin-report');if(rescuePanel)rescuePanel.classList.toggle('hidden',key!=='rescue');const roulettePanel=document.getElementById('mz-roulette-admin-report');if(roulettePanel)roulettePanel.classList.toggle('hidden',key!=='roulette');const envelopePanel=document.getElementById('mz-envelope-admin-report');if(envelopePanel)envelopePanel.classList.toggle('hidden',key!=='envelope');const cashwheelPanel=document.getElementById('mz-cashwheel-admin-report');if(cashwheelPanel)cashwheelPanel.classList.toggle('hidden',key!=='cashwheel');const lotteryPanel=document.getElementById('mz-lottery-admin-report');if(lotteryPanel)lotteryPanel.classList.toggle('hidden',key!=='lottery');if(key==='roulette')loadRouletteReport();if(key==='envelope')loadEnvelopeReport();if(key==='cashwheel')loadCashwheelReport();if(key==='lottery')loadLotteryReport();const supported=!!definitions[key];$id('admin-promotion-detail').classList.toggle('rebate-fixed-mode',key==='rebate');$id('promotion-config-new').classList.toggle('hidden',!supported);$id('promotion-config-head').replaceChildren();$id('promotion-config-body').replaceChildren();$id('promotion-config-alert').classList.add('hidden');
...
(() => {
 ...
 async function overview(){const status=el('mz-oversight-message');status.textContent='Atualizando dados...';try{
   ...
   const claimedCount=(d.redemptions||[]).reduce((v,r)=>v+Number(r.total||0),0);const metrics=d.metrics||{};
   for(const [label,val] of [['Campanhas cadastradas',campaignCount],['Configurações habilitadas',enabledCount],['Resgates registrados',claimedCount],['Jogadores com resgate',Number(metrics.players||0)],['Jogadores ativos 30d',Number(metrics.players_30d||0)],['Valor nominal registrado',moneyValue(metrics.nominal_minor||0)]]){const card=document.createElement('div');add(card,'span',label);add(card,'strong',val);stats.append(card);}
   ...
   const configBody=el('mz-oversight-configs');configBody.replaceChildren();for(const row of d.configurations||[]){const tr=document.createElement('tr');add(tr,'td',row.type);add(tr,'td',row.title);const tdStatus=add(tr,'td',Number(row.enabled)?'Ativa':'Desativada');tdStatus.className=Number(row.enabled)?'mz-config-on':'mz-config-off';const td=document.createElement('td');const btn=document.createElement('button');btn.type='button';btn.className='secondary mz-config-toggle';btn.textContent=Number(row.enabled)?'DESATIVAR':'ATIVAR';btn.dataset.id=row.id;btn.dataset.type=row.type;btn.dataset.enabled=Number(row.enabled)?'0':'1';td.append(btn);tr.append(td);configBody.append(tr);}
 ...
 async function audit(){const status=el('mz-promo-audit-message'),root=el('mz-promo-audit-results');root.replaceChildren();status.textContent='Executando consultas locais de leitura...';try{
   const d=await api('/admin/api/promotions/financial-audit');const names={wallet_ledger_mismatch:'Carteira x saldo líquido no ledger',invalid_ledger_transition:'Movimentação com cálculo inconsistente',redemption_missing_transaction:'Resgate sem transação concluída',invalid_rollover_progress:'Progresso de rollover inconsistente',duplicate_redemption_transactions:'Transação vinculada a mais de um resgate',overallocated_bet:'Aposta alocada acima do valor real',rollover_allocation_mismatch:'Rollover divergente das apostas alocadas',duplicate_promotion_credit_reference:'Crédito de promoção repetido por referência',negative_wallet_balance:'Carteira com saldo negativo',completed_financial_without_ledger:'Transação concluída sem lançamento no ledger',promotion_transaction_amount_mismatch:'Valor do resgate divergente do ledger'};
 ...
 el('mz-oversight-configs')?.addEventListener('click',async event=>{const button=event.target.closest('.mz-config-toggle');if(!button)return;button.disabled=true;try{await api('/admin/api/promotion-configs/toggle',{method:'POST',body:JSON.stringify({id:Number(button.dataset.id),type:button.dataset.type,enabled:button.dataset.enabled==='1'})});await overview();}catch(error){el('mz-oversight-message').textContent='Falha ao alterar campanha: '+error.message;button.disabled=false;}});
 el('mz-player-diagnostic-form')?.addEventListener('submit',async event=>{event.preventDefault();const input=el('mz-player-diagnostic-input'),root=el('mz-player-diagnostic-result');root.replaceChildren();const player=input.value.trim();if(!player){add(root,'p','Informe um jogador.');return;}add(root,'p','Consultando...','hint');try{const d=await api('/admin/api/promotions/player-diagnostic?player='+encodeURIComponent(player));root.replaceChildren();if(!d.found){add(root,'p',d.message||'Jogador não encontrado.');return;}const head=document.createElement('div');head.className='mz-diagnostic-head';add(head,'strong','#'+d.user.public_id+' • '+d.user.username);add(head,'small',d.user.status+' • '+d.redemptions+' resgate(s) registrado(s)');root.append(head);const grid=document.createElement('div');grid.className='mz-diagnostic-grid';for(const mod of d.modules||[]){const card=document.createElement('div');card.className='mz-diagnostic-card '+(mod.available?'is-ready':'is-off');add(card,'strong',mod.title);add(card,'span',mod.available?'Disponível':'Indisponível');add(card,'small',mod.detail);grid.append(card);}root.append(grid);add(root,'p',d.note||'','hint');}catch(error){root.replaceChildren();add(root,'p','Falha no diagnóstico: '+error.message);}});
 // Atualiza o resumo ao entrar em Promoções, sem executar automaticamente as consultas pesadas de auditoria.
 document.querySelectorAll('[data-page="promotions"],[data-admin-promotion-tab="central"]').forEach(b=>b.addEventListener('click',()=>{if(!el('page-promotions').classList.contains('hidden'))overview();}));
})();


// V24.1 • Central administrativa de notificações
const notificationCategoryLabel=value=>({announcement:'Anúncios',financial:'Financeiro',promotion:'Promoções',security:'Segurança',support:'Suporte',system:'Sistema',user:'Usuário'})[String(value)]||String(value||'—');
const notificationPriorityLabel=value=>({normal:'Normal',high:'Alta',urgent:'Urgente'})[String(value)]||'Normal';
function resetNotificationForm(){const form=$('#notification-form');if(!form)return;form.reset();form.elements.id.value='';form.elements.enabled.checked=true;form.elements.audience.value='ALL';form.elements.priority.value='normal';$('.notification-user-target')?.classList.add('hidden');}
function notificationPayload(row){return {id:row.id,category:row.category,priority:row.priority||'normal',audience:row.audience,user_identifier:row.user_id||'',title:row.title,message:row.message,link_path:row.link_path||'',starts_at:row.starts_at?String(row.starts_at).replace(' ','T').slice(0,16):'',ends_at:row.ends_at?String(row.ends_at).replace(' ','T').slice(0,16):'',enabled:!!Number(row.enabled)};}
function editNotification(row){const f=$('#notification-form');if(!f)return;const data=notificationPayload(row);for(const [key,value] of Object.entries(data)){if(!f.elements[key])continue;if(f.elements[key].type==='checkbox')f.elements[key].checked=!!value;else f.elements[key].value=value??'';}$('.notification-user-target')?.classList.toggle('hidden',data.audience!=='USER');f.scrollIntoView({behavior:'smooth',block:'start'});}
async function loadNotifications(){const body=$('#notifications-body');if(!body)return;body.innerHTML='<tr><td colspan="7">Carregando...</td></tr>';try{const data=await api('/admin/api/notifications');body.replaceChildren();for(const item of data.items||[]){const tr=document.createElement('tr');const audience=item.audience==='USER'?`#${item.public_id||'—'} · ${item.username||'Usuário'}`:'Todos os jogadores';const period=[item.starts_at?dateTime(item.starts_at):'Agora',item.ends_at?dateTime(item.ends_at):'Sem término'].join(' → ');for(const value of [notificationCategoryLabel(item.category),notificationPriorityLabel(item.priority),audience,item.title,item.enabled?'Publicada':'Desativada',period]){const td=document.createElement('td');td.textContent=String(value);tr.append(td)}const actions=document.createElement('td');const edit=document.createElement('button');edit.type='button';edit.className='secondary';edit.textContent='Editar';edit.addEventListener('click',()=>editNotification(item));const del=document.createElement('button');del.type='button';del.className='danger-soft';del.textContent='Excluir';del.addEventListener('click',async()=>{if(!confirm('Excluir esta notificação?'))return;del.disabled=true;try{await api('/admin/api/notifications/delete',{method:'POST',body:JSON.stringify({id:item.id})});await loadNotifications();}catch(error){alertBox('#notifications-alert',error.message);del.disabled=false;}});actions.append(edit,del);tr.append(actions);body.append(tr);}if(!data.items?.length){const tr=document.createElement('tr');const td=document.createElement('td');td.colSpan=7;td.textContent='Nenhuma notificação cadastrada.';tr.append(td);body.append(tr)}}catch(error){body.innerHTML='<tr><td colspan="7">Falha ao carregar notificações.</td></tr>';alertBox('#notifications-alert',error.message)}}
$('#notification-form')?.elements.audience?.addEventListener('change',event=>$('.notification-user-target')?.classList.toggle('hidden',event.target.value!=='USER'));
$('#notification-form')?.addEventListener('submit',async event=>{event.preventDefault();const form=event.currentTarget;const data=Object.fromEntries(new FormData(form));data.enabled=form.elements.enabled.checked;const button=form.querySelector('button[type=submit]');button.disabled=true;try{await api('/admin/api/notifications/save',{method:'POST',body:JSON.stringify(data)});alertBox('#notifications-alert','Notificação salva com sucesso.',true);resetNotificationForm();await loadNotifications();}catch(error){alertBox('#notifications-alert',error.message)}finally{button.disabled=false}});
$('#notification-form-reset')?.addEventListener('click',resetNotificationForm);
$('#notifications-refresh')?.addEventListener('click',loadNotifications);


const notificationAutomationLabel=value=>({
  deposit_created:'Depósito • PIX criado',deposit_paid:'Depósito • confirmado',deposit_failed:'Depósito • falhou/expirou',first_deposit_bonus:'Primeiro depósito • bônus creditado',
  withdrawal_created:'Saque • solicitado',withdrawal_processing:'Saque • processamento',withdrawal_paid:'Saque • concluído',withdrawal_failed:'Saque • falhou/cancelado',
  promo_roulette_available:'Promoção • Giro da Sorte',promo_cashwheel_available:'Promoção • Roleta de Saque',promo_lottery_available:'Promoção • Sorteio de Cartas',promo_chest_available:'Promoção • Baú do Tesouro',promo_rebate_available:'Promoção • Rebate',promo_rescue_available:'Promoção • Fundo de Resgate',vip_available:'Promoção • Benefício VIP'
})[String(value)]||String(value||'—');
let notificationAutomationRows=[];
function editNotificationAutomation(row){const f=$('#notification-automation-form');if(!f)return;f.classList.remove('hidden');f.elements.event_key.value=row.event_key||'';f.elements.event_label.value=notificationAutomationLabel(row.event_key);f.elements.category.value=row.category||'system';f.elements.priority.value=row.priority||'normal';f.elements.title_template.value=row.title_template||'';f.elements.message_template.value=row.message_template||'';f.elements.link_path.value=row.link_path||'';f.elements.enabled.checked=!!Number(row.enabled);f.scrollIntoView({behavior:'smooth',block:'center'});}
async function loadNotificationAutomations(){const body=$('#notification-automations-body');if(!body)return;body.innerHTML='<tr><td colspan="6">Carregando...</td></tr>';try{const data=await api('/admin/api/notification-automations');notificationAutomationRows=data.items||[];body.replaceChildren();for(const item of notificationAutomationRows){const tr=document.createElement('tr');for(const value of [notificationAutomationLabel(item.event_key),notificationCategoryLabel(item.category),notificationPriorityLabel(item.priority),Number(item.enabled)?'Ativa':'Desativada',item.title_template]){const td=document.createElement('td');td.textContent=String(value);tr.append(td)}const actions=document.createElement('td');const edit=document.createElement('button');edit.type='button';edit.className='secondary';edit.textContent='Editar';edit.addEventListener('click',()=>editNotificationAutomation(item));actions.append(edit);tr.append(actions);body.append(tr)}if(!notificationAutomationRows.length){const tr=document.createElement('tr');const td=document.createElement('td');td.colSpan=6;td.textContent='Nenhuma automação configurada. Execute as migrations.';tr.append(td);body.append(tr)}}catch(error){body.innerHTML='<tr><td colspan="6">Falha ao carregar automações.</td></tr>';alertBox('#notification-automations-alert',error.message)}}
$('#notification-automation-form')?.addEventListener('submit',async event=>{event.preventDefault();const form=event.currentTarget;const data=Object.fromEntries(new FormData(form));delete data.event_label;data.enabled=form.elements.enabled.checked;const button=form.querySelector('button[type=submit]');button.disabled=true;try{await api('/admin/api/notification-automations/save',{method:'POST',body:JSON.stringify(data)});alertBox('#notification-automations-alert','Automação atualizada com sucesso.',true);form.classList.add('hidden');await loadNotificationAutomations();}catch(error){alertBox('#notification-automations-alert',error.message)}finally{button.disabled=false}});
$('#notification-automation-cancel')?.addEventListener('click',()=>$('#notification-automation-form')?.classList.add('hidden'));
$('#notification-automations-refresh')?.addEventListener('click',loadNotificationAutomations);



// V24.7 • Navegação segmentada de Notificações
function selectNotificationAdminTab(tab='central'){
  document.querySelectorAll('[data-notification-panel]').forEach(panel=>panel.classList.toggle('hidden',panel.dataset.notificationPanel!==tab));
  document.querySelectorAll('[data-admin-notification-tab]').forEach(button=>button.classList.toggle('active',button.dataset.adminNotificationTab===tab));
  if(tab==='manual')loadNotifications();else if(tab==='automations')loadNotificationAutomations();
}
async function loadNotificationCentral(){
  const group=document.querySelector('.notification-admin-group'),parent=document.querySelector('.notification-admin-parent');
  group?.classList.remove('collapsed');parent?.setAttribute('aria-expanded','true');selectNotificationAdminTab('central');
  try{const [manual,auto]=await Promise.all([api('/admin/api/notifications'),api('/admin/api/notification-automations')]);const items=manual.items||[],autos=auto.items||[];const set=(id,v)=>{const el=document.querySelector(id);if(el)el.textContent=String(v)};set('#notification-summary-total',items.length);set('#notification-summary-active',items.filter(x=>Number(x.enabled)).length);set('#notification-summary-auto',autos.filter(x=>Number(x.enabled)).length);set('#notification-summary-priority',items.filter(x=>['high','urgent'].includes(String(x.priority))).length);}catch(error){alertBox('#notifications-alert',error.message)}
}
(()=>{const group=document.querySelector('.notification-admin-group'),parent=document.querySelector('.notification-admin-parent');if(!group||!parent)return;parent.addEventListener('click',e=>{e.stopImmediatePropagation();const collapsed=group.classList.toggle('collapsed');parent.setAttribute('aria-expanded',String(!collapsed));if(!collapsed){openPage('notifications');selectNotificationAdminTab('central')}});document.querySelectorAll('[data-admin-notification-tab]').forEach(button=>button.addEventListener('click',e=>{e.stopPropagation();group.classList.remove('collapsed');parent.setAttribute('aria-expanded','true');openPage('notifications');selectNotificationAdminTab(button.dataset.adminNotificationTab)}));})();

// V24.7 • Central de Suporte
const supportCategoryLabel=v=>({deposit:'Depósito',withdrawal:'Saque',bonus:'Bônus',account:'Conta',games:'Jogos',other:'Outros'})[String(v)]||String(v||'—');
const supportStatusLabel=v=>({OPEN:'Aberto',IN_PROGRESS:'Em atendimento',RESOLVED:'Resolvido',CLOSED:'Fechado'})[String(v)]||String(v||'—');
const supportPriorityLabel=v=>({normal:'Normal',high:'Alta',urgent:'Urgente'})[String(v)]||String(v||'—');
let supportAdminCurrent=null;
async function loadSupportTickets(){const body=$('#support-admin-body');if(!body)return;body.innerHTML='<tr><td colspan="8">Carregando...</td></tr>';try{const q=new URLSearchParams();const search=$('#support-admin-search')?.value.trim();const status=$('#support-admin-status')?.value;const category=$('#support-admin-category')?.value;if(search)q.set('q',search);if(status)q.set('status',status);if(category)q.set('category',category);const data=await api('/admin/api/support/tickets?'+q.toString());body.replaceChildren();for(const item of data.items||[]){const tr=document.createElement('tr');const sla=Number(item.sla_hours||0);for(const value of [item.ticket_code,`#${item.public_id||'—'} · ${item.username||'—'}`,supportCategoryLabel(item.category),supportPriorityLabel(item.priority),supportStatusLabel(item.status),sla?`${sla}h`:'—',dateTime(item.updated_at)]){const td=document.createElement('td');td.textContent=String(value);if(value===`${sla}h`&&sla>=24)td.className='support-sla-late';tr.append(td)}const td=document.createElement('td');const b=document.createElement('button');b.type='button';b.className='secondary';b.textContent='Abrir';b.addEventListener('click',()=>openSupportTicket(item.id));td.append(b);tr.append(td);body.append(tr)}if(!data.items?.length){const tr=document.createElement('tr');const td=document.createElement('td');td.colSpan=8;td.textContent='Nenhum ticket encontrado.';tr.append(td);body.append(tr)}}catch(error){body.innerHTML='<tr><td colspan="8">Falha ao carregar tickets.</td></tr>';alertBox('#support-admin-alert',error.message)}}
function renderSupportAdminMessages(ticket){const host=$('#support-admin-messages');host.replaceChildren();for(const msg of ticket.messages||[]){const item=document.createElement('article');item.className='support-message '+(msg.sender_type==='USER'?'from-user':msg.sender_type==='INTERNAL'?'is-internal':'from-admin');const head=document.createElement('div');head.className='support-message-head';const who=msg.sender_type==='USER'?'Jogador':msg.sender_type==='INTERNAL'?'Observação interna':'Suporte';head.innerHTML=`<strong>${who}</strong><small>${dateTime(msg.created_at)}</small>`;const p=document.createElement('p');p.textContent=msg.message;item.append(head,p);if(msg.attachment_path){const a=document.createElement('a');a.href=msg.attachment_path;a.target='_blank';a.rel='noopener noreferrer';a.textContent='Ver anexo';item.append(a)}host.append(item)}}
async function openSupportTicket(id){try{const t=await api('/admin/api/support/ticket?id='+encodeURIComponent(id));supportAdminCurrent=t;$('#support-admin-detail').classList.remove('hidden');$('#support-admin-detail-title').textContent=t.ticket_code+' • '+t.subject;$('#support-admin-detail-meta').textContent=`#${t.public_id} · ${t.username} · ${supportCategoryLabel(t.category)} · SLA ${Number(t.sla_hours||0)}h`;$('#support-admin-detail-status').value=t.status;$('#support-admin-detail-priority').value=t.priority;$('#support-admin-reply-form').elements.ticket_id.value=t.id;renderSupportAdminMessages(t);$('#support-admin-detail').scrollIntoView({behavior:'smooth',block:'start'});}catch(error){alertBox('#support-admin-alert',error.message)}}
$('#support-admin-refresh')?.addEventListener('click',loadSupportTickets);$('#support-admin-search-btn')?.addEventListener('click',loadSupportTickets);$('#support-admin-search')?.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();loadSupportTickets()}});$('#support-admin-detail-close')?.addEventListener('click',()=>$('#support-admin-detail')?.classList.add('hidden'));
$('#support-admin-update')?.addEventListener('click',async()=>{if(!supportAdminCurrent)return;try{const t=await api('/admin/api/support/update',{method:'POST',body:JSON.stringify({ticket_id:supportAdminCurrent.id,status:$('#support-admin-detail-status').value,priority:$('#support-admin-detail-priority').value})});supportAdminCurrent=t;renderSupportAdminMessages(t);await loadSupportTickets();alertBox('#support-admin-alert','Situação do ticket atualizada.',true);}catch(error){alertBox('#support-admin-alert',error.message)}});
async function uploadAdminSupportImage(file){if(!file)return '';const form=new FormData();form.append('image',file);const headers={Accept:'application/json'};if(token)headers.Authorization=`Bearer ${token}`;const res=await fetch('/admin/api/support/upload',{method:'POST',headers,body:form});const data=await res.json().catch(()=>({}));if(!res.ok)throw new Error(data.message||data.error||'Falha no envio do anexo.');return data.path||'';}
$('#support-admin-reply-form')?.addEventListener('submit',async e=>{e.preventDefault();const f=e.currentTarget,fd=new FormData(f);const button=f.querySelector('button[type=submit]');button.disabled=true;try{const file=fd.get('image');const attachment=file instanceof File&&file.size?await uploadAdminSupportImage(file):'';const data={ticket_id:Number(fd.get('ticket_id')),message:fd.get('message'),internal:f.elements.internal.checked,attachment_path:attachment};const t=await api('/admin/api/support/reply',{method:'POST',body:JSON.stringify(data)});supportAdminCurrent=t;f.elements.message.value='';f.elements.image.value='';f.elements.internal.checked=false;renderSupportAdminMessages(t);await loadSupportTickets();alertBox('#support-admin-alert',data.internal?'Observação interna adicionada.':'Resposta enviada e jogador notificado.',true);}catch(error){alertBox('#support-admin-alert',error.message)}finally{button.disabled=false}});
