...
const notificationCategoryLabel=value=>({announcement:'Anúncios',system:'Sistema',user:'Usuário',support:'Suporte'})[String(value)]||String(value||'—');
function resetNotificationForm(){const form=$('#notification-form');if(!form)return;form.reset();form.elements.id.value='';form.elements.enabled.checked=true;form.elements.audience.value='ALL';$('.notification-user-target')?.classList.add('hidden');}
function notificationPayload(row){return {id:row.id,category:row.category,audience:row.audience,user_identifier:row.user_id||'',title:row.title,message:row.message,link_path:row.link_path||'',starts_at:row.starts_at?String(row.starts_at).replace(' ','T').slice(0,16):'',ends_at:row.ends_at?String(row.ends_at).replace(' ','T').slice(0,16):'',enabled:!!Number(row.enabled)};}
...
async function loadNotifications(){const body=$('#notifications-body');if(!body)return;body.innerHTML='<tr><td colspan="6">Carregando...</td></tr>';try{const data=await api('/admin/api/notifications');body.replaceChildren();for(const item of data.items||[]){const tr=document.createElement('tr');const audience=item.audience==='USER'?`#${item.public_id||'—'} · ${item.username||'Usuário'}`:'Todos os jogadores';const period=[item.starts_at?dateTime(item.starts_at):'Agora',item.ends_at?dateTime(item.ends_at):'Sem término'].join(' → ');for(const value of [notificationCategoryLabel(item.category),audience,item.title,item.enabled?'Publicada':'Desativada',period]){const td=document.createElement('td');td.textContent=String(value);tr.append(td)}const actions=document.createElement('td');const edit=document.createElement('button');edit.type='button';edit.className='secondary';edit.textContent='Editar';edit.addEventListener('click',()=>editNotification(item));const del=document.createElement('button');del.type='button';del.className='danger-soft';del.textContent='Excluir';del.addEventListener('click',async()=>{if(!confirm('Excluir esta notificação?'))return;del.disabled=true;try{await api('/admin/api/notifications/delete',{method:'POST',body:JSON.stringify({id:item.id})});await loadNotifications();}catch(error){alertBox('#notifications-alert',error.message);del.disabled=false;}});actions.append(edit,del);tr.append(actions);body.append(tr);}if(!data.items?.length){const tr=document.createElement('tr');const td=document.createElement('td');td.colSpan=6;td.textContent='Nenhuma notificação cadastrada.';tr.append(td);body.append(tr)}}catch(error){body.innerHTML='<tr><td colspan="6">Falha ao carregar notificações.</td></tr>';alertBox('#notifications-alert',error.message)}}
