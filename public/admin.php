...
<title><?=ah($appName)?> Admin</title>
<link rel="stylesheet" href="/admin-assets/admin.css?v=20260917-mz90-v11-2-layout-login-capas">
</head>
<body>
<div class="admin-shell login-mode" id="admin-shell">
<aside class="sidebar hidden" id="admin-sidebar">
    <div class="admin-brand"><b>M</b><span><?=ah($appName)?><small>ADMIN</small></span></div>
    <nav class="admin-nav">
        ...
        <div class="casino-nav-group"><button class="nav-item casino-nav-parent" type="button" data-page="casino" aria-expanded="true"><span>♠</span> Jogos <span class="casino-nav-chevron">⌄</span></button><div class="casino-subnav"><button type="button" class="casino-subitem" data-casino-tab="games">♧ Catálogo de jogos</button><button type="button" class="casino-subitem" data-casino-tab="providers">▣ Provedores</button><button type="button" class="casino-subitem" data-casino-tab="credentials">⚿ Credenciais das APIs</button><button type="button" class="casino-subitem" data-casino-tab="bets" disabled title="Depende de integração real">◴ Histórico de apostas <small>Em preparação</small></button><button type="button" class="casino-subitem" data-casino-tab="rtp" disabled title="Depende de apostas liquidadas">▥ RTP observado <small>Em preparação</small></button></div></div>
        <button class="nav-item" data-page="promotions"><span>◆</span> Promoções</button>
...
<section id="admin-app" class="hidden">
    ...
    <section id="page-casino" class="admin-page hidden">
      ...
      <div class="dashboard-actions"><span>Catálogo e organização • jogos reais não são iniciados nesta versão.</span><button id="casino-refresh" class="secondary-action">Atualizar</button></div>
      ...
      <div class="table-wrap"><table><thead><tr><th>Código</th><th>Nome</th><th>Modo</th><th>API</th><th>Status</th><th></th></tr></thead><tbody id="casino-providers-body"></tbody></table></div></article>
      <article class="panel casino-panel hidden" id="casino-credentials-panel"><div class="panel-title"><h2>PlayFiver · Credenciais</h2><span>As chaves ficam criptografadas. Apostas reais permanecem bloqueadas.</span></div>
      <div id="playfiver-alert" class="alert hidden"></div>
      <p id="playfiver-status">Carregando configuração...</p>
      <form id="playfiver-form" class="casino-form" autocomplete="off"><label>URL da API<input name="base_url" type="url" required value="https://api.playfivers.com"></label><label>Agent Code<input name="agent_code" maxlength="1024" autocomplete="off" placeholder="Em branco: preservar credencial salva"></label><label>Agent Token<input name="agent_token" type="password" maxlength="1024" autocomplete="new-password" placeholder="Em branco: preservar credencial salva"></label><label>Agent Secret<input name="agent_secret" type="password" maxlength="1024" autocomplete="new-password" placeholder="Em branco: preservar credencial salva"></label><button type="submit" class="gold-button casino-save">SALVAR CREDENCIAIS</button></form>
      <p>Integração desativada até validação de abertura de jogos, callbacks e carteira. Esta tela não executa testes de produção.</p></article>
      <article class="panel casino-panel games-api-panel" id="casino-games-panel">
        <div class="panel-title games-api-title"><div><h2>Gerenciamento de Jogos API</h2><span>Visualize e gerencie os jogos cadastrados por provedor e API.</span></div><button type="button" class="gold-button" id="new-game">＋ Adicionar Jogo</button></div>
        ...
        <div class="table-wrap games-api-table-wrap"><table class="games-api-table"><thead><tr><th>Banner</th><th>Nome</th><th>Código</th><th>Provider</th><th>API</th><th>Status</th><th>Popular</th><th>Ações</th></tr></thead><tbody id="casino-games-body"></tbody></table></div>
        ...
        <div class="games-api-info"><strong>ⓘ Informações</strong><div><span>Jogos cadastrados no catálogo local; sincronização automática com APIs ainda não habilitada.</span><span>Use a busca para localizar jogos rapidamente.</span><span>Marque como Popular para destacar na home.</span><span>Desativar o status oculta o jogo do catálogo público.</span></div></div>
      </article>
    </section>
...
<div class="identity-form-actions"><button type="submit" class="gold-button">Salvar identidade visual</button></div></form></article></div>
...
<script src="/admin-assets/admin.js?v=20260917-mz90-v11-3-promotions-only"></script>
