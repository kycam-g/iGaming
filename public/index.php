...
$router->post('/admin/api/platform/floating-icons/save',function(Request $request) use($adminAuth,$platform,$audit){$admin=$adminAuth->authenticate($request->bearerToken());if(!$admin)Response::json(['error'=>'unauthorized'],401);$result=$platform->saveFloatingIcon($request->body);$audit->record('ADMIN',(string)$admin['id'],'platform.floating_icon_saved','floating_icons',(string)$result['id'],$request->clientIp(),['module_id'=>$result['module_id'],'show_when'=>$result['show_when']]);return $result;});
$router->post('/admin/api/platform/floating-icons/delete',function(Request $request) use($adminAuth,$platform,$audit){$admin=$adminAuth->authenticate($request->bearerToken());if(!$admin)Response::json(['error'=>'unauthorized'],401);$id=(string)($request->body['id']??'');$platform->deleteFloatingIcon($id);$audit->record('ADMIN',(string)$admin['id'],'platform.floating_icon_deleted','floating_icons',$id,$request->clientIp());return ['ok'=>true];});

$router->get('/api/notifications',function(Request $request)use($auth,$notifications){$user=$auth->authenticate($request->bearerToken());if(!$user)Response::json(['error'=>'unauthorized'],401);return $notifications->forUser((string)$user['id']);});
$router->post('/api/notifications/read',function(Request $request)use($auth,$notifications){$user=$auth->authenticate($request->bearerToken());if(!$user)Response::json(['error'=>'unauthorized'],401);$notifications->markRead((string)$user['id'],(int)($request->body['id']??0));return ['ok'=>true];});
$router->post('/api/notifications/read-all',function(Request $request)use($auth,$notifications){$user=$auth->authenticate($request->bearerToken());if(!$user)Response::json(['error'=>'unauthorized'],401);$notifications->markAllRead((string)$user['id']);return ['ok'=>true];});
$router->get('/admin/api/notifications',function(Request $request)use($adminAuth,$notifications){if(!$adminAuth->authenticate($request->bearerToken()))Response::json(['error'=>'unauthorized'],401);return ['items'=>$notifications->adminList()];});
$router->post('/admin/api/notifications/save',function(Request $request)use($adminAuth,$notifications,$audit){$admin=$adminAuth->authenticate($request->bearerToken());if(!$admin)Response::json(['error'=>'unauthorized'],401);$result=$notifications->save($request->body);$audit->record('ADMIN',(string)$admin['id'],'platform.notification_saved','platform_notifications',(string)($result['id']??''),$request->clientIp(),['category'=>$result['category']??null,'audience'=>$result['audience']??null]);return $result;});
...
$router->get('/admin/api/promotions/player-diagnostic',function(Request $request)use($adminAuth,$promotionOversight){if(!$adminAuth->authenticate($request->bearerToken()))Response::json(['error'=>'unauthorized'],401);return $promotionOversight->diagnostic((string)($request->query['player']??''));});
...
$router->post('/admin/api/promotion-configs/toggle',function(Request $request) use($adminAuth,$promotionConfigs,$audit){$admin=$adminAuth->authenticate($request->bearerToken());if(!$admin)Response::json(['error'=>'unauthorized'],401);$type=(string)($request->body['type']??'');$id=(int)($request->body['id']??0);$enabled=filter_var($request->body['enabled']??false,FILTER_VALIDATE_BOOLEAN);$result=$promotionConfigs->toggleEnabled($type,$id,$enabled);$audit->record('ADMIN',(string)$admin['id'],'promotion.configuration_toggled',$type,(string)$id,$request->clientIp(),['enabled'=>$enabled]);return $result;});
...
$router->post('/admin/api/platform/upload',function(Request $request) use($adminAuth,$audit){$admin=$adminAuth->authenticate($request->bearerToken());if(!$admin)Response::json(['error'=>'unauthorized'],401);$kind=(string)($_POST['kind']??'');if(!in_array($kind,['banners','promotions','identity','providers','floats'],true))Response::json(['error'=>'invalid_kind'],422);$file=$_FILES['image']??null;if(!$file||$file['error']!==UPLOAD_ERR_OK||$file['size']>($kind==='floats'?5242880:2097152))Response::json(['error'=>'invalid_file'],422);$mime=(new finfo(FILEINFO_MIME_TYPE))->file($file['tmp_name']);$ext=match($mime){'image/jpeg'=>'jpg','image/png'=>'png','image/webp'=>'webp','image/gif'=>'gif',default=>null};if(!$ext||!getimagesize($file['tmp_name']))Response::json(['error'=>'invalid_image'],422);$dir=__DIR__.'/uploads/'.$kind;if(!is_dir($dir)&&!mkdir($dir,0755,true)&&!is_dir($dir))Response::json(['error'=>'upload_failed'],500);$name=bin2hex(random_bytes(16)).'.'.$ext;if(!move_uploaded_file($file['tmp_name'],$dir.'/'.$name))Response::json(['error'=>'upload_failed'],500);$audit->record('ADMIN',(string)$admin['id'],'platform.image_uploaded',$kind,$name,$request->clientIp());return ['path'=>'/uploads/'.$kind.'/'.$name];});
...
$router->get('/api/payments/gateways', function(Request $request) use($payments,$auth){$user=$auth->authenticate($request->bearerToken());return ['gateways'=>$payments->availableGateways('DEPOSIT'),'offer'=>$payments->depositOffer($user['id']??null)];});
