...
$router->get('/api/platform/public', fn() => ['settings'=>array_intersect_key($platform->settings(),array_flip(['site_name','accent_color','footer_text'])),'banners'=>$platform->list('banners',true),'promotions'=>$platform->list('promotions',true)]);
$router->get('/admin/api/platform', function(Request $request) use($adminAuth,$platform){if(!$adminAuth->authenticate($request->bearerToken()))Response::json(['error'=>'unauthorized'],401);return ['settings'=>$platform->settings(),'banners'=>$platform->list('banners'),'promotions'=>$platform->list('promotions'),'affiliates'=>$platform->affiliateReport()];});
...
$router->get('/admin/api/casino/catalog', function (Request $request) use ($adminAuth,$casino) { if (!$adminAuth->authenticate($request->bearerToken())) Response::json(['error'=>'unauthorized'],401); return ['providers'=>$casino->providers(),'games'=>$casino->games()]; });
