(() => {
  ...
  function section(name){$$('.page-section').forEach(x=>x.classList.toggle('active',x.id===`section-${name}`));$$('.bottom-item[data-section]').forEach(x=>x.classList.toggle('active',x.dataset.section===name));window.scrollTo({top:0,behavior:'smooth'});if(name==='profile')loadWallet();document.dispatchEvent(new CustomEvent('mz:section-changed',{detail:{name}}))}
